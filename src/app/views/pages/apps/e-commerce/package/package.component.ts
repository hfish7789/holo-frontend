		...
		this.packagesService.getAllPackages().then(async result => {
			...
			await this.packages.map(async result => {
				this.ordersService.getOrderByID(result.idorders).then(async results => {
          			result.orderStatus = results[0]?.status ? results[0]?.status : "";
					await this.packagesService.getCustomerByorderID(result.idorders).then(async results => {
						result.customer = results[0]?.firstName ? results[0]?.firstName : "";
						await this.packagesService.getMessengerByorderID(result.idorders).then(results => {
							result.assignedTo = results[0]?.name ? results[0]?.name : "";
							this.packages = this.packages.filter((item) => item.orderStatus != '3')
					...
					if (result.status == 0 && result.orderStatus != '3') { this.porRecoger++ }
					if (result.status == 1 && result.orderStatus != '3') { this.recogido++ }
					if (result.status == 2 && result.orderStatus != '3') { this.bodega++ }
					if (result.status == 3 && result.orderStatus != '3') { this.porEntregar++ }
				});
			})
		})
	}
