...
export class OrderCreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log(9999,this.packageForm.value.packages)
  }
  ...
  checkValidPeso(i) {
    console.log('han11111',this.selectedValue);
    console.log('zi111111',this.getControls()[i].get('weight').value <= 3);
    if(this.selectedValue === '1') {
      if(this.getControls()[i].get('weight').value <= 3) {
        return true;
      } else {
        return false;
      }
  ...
  calcPrice(data="", i=0){
    if(data){
      if(data != "1"){
        this.getControls()[i].get('weight').setValidators([
            Validators.required,
            Validators.min(0),
            Validators.max(3)
        ])
      ...
      {
        this.getControls()[i].get('weight').clearValidators();
      }
      this.getControls()[i].get('weight').updateValueAndValidity();
    }
    ...
    && this.packageForm.value.packages[0].type != null && this.packageForm.value.packages[0].type != undefined && this.packageForm.value.packages[0].type != ''
      ...
      this.priceService.getPrice(this.customerGroup, this.pickUpAddressForm.value.pickUpAddressArea, this.packageForm.value.packages[0].type).then((result:any) => {
        ...
        this.priceService.getPrice(this.customerGroup, this.deliveryAddressForm.value.deliveryAddressArea, this.packageForm.value.packages[0].type).then((result:any) => {
          ...
          if(this.typeOfService){
						this.totalPrice = deliveryPrice + Number(deliveryAreaPrice?.locationPrice)*Number(this.packageForm.value.packages.length);
          }
          else {
            ...
            if (Number(deliveryAreaPrice?.locationPrice) > Number(pickUpAreaPrice?.locationPrice)) {
							this.totalPrice = deliveryPrice + pickUpPrice + Number(deliveryAreaPrice?.locationPrice)*Number(this.packageForm.value.packages.length);
						} else {
							this.totalPrice = deliveryPrice + pickUpPrice + Number(pickUpAreaPrice?.locationPrice)*Number(this.packageForm.value.packages.length);
						}
          }
